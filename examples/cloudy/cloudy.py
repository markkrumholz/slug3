...
def _(plt, sim_output):
    plt.loglog(sim_output.cluster_spectra['wl'], sim_output.cluster_spectra['spec'][0,:])
    plt.loglog(sim_output.cluster_cloudy['wl'], sim_output.cluster_cloudy['spec_trans_emit'][0,:])
    plt.loglog(sim_output.cluster_cloudy['wl'], sim_output.cluster_cloudy['spec_inc'][0,:])
    return
...
@app.cell
def _(c, h, sim_output, u):
    (h*c/sim_output.cluster_cloudy['wl'][0]).to(u.Ry)
    return
