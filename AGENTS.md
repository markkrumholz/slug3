...
* CI's `pyright` job, like `clang-tidy`'s, only checks Python files under `slugpy`/`tests/slugpy` that changed relative to the PR's base branch, not the whole tree. Before pushing, check your own changed files the same way -- e.g. `pyright --pythonpath "$(which python3)" $(git diff --name-only origin/main...HEAD -- slugpy tests/slugpy)` -- with `pyright` built against the compiled `_slug` extension already present (`cmake --build build --target slugPython`), since every `slugpy` module imports from it.
