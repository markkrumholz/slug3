...
namespace tracks
{
            ...
            -> std::pair<std::vector<double>, std::vector<double>>
        {
            ...
            for (size_t f = 0; f < nfeh; ++f)
            {
                ...
                for (size_t i = 0; i < nmass; ++i)
                {
                    ...
                    auto [data, shape] = utils::readDataset2D(grp, name, "Tracks3D");
                    ...
                    for (size_t j = 1; j < nt; ++j)
                    {
                        ...
                        // clamp to a 1e-6 yr floor before taking log10
                        // so that this maps to a large-but-finite
                        // negative value instead of -inf, which would
                        // otherwise poison Mesh3DInterpolator's (or the
                        // Mesh2DInterpolator slices it builds on)
                        // arc-length computation with NaN. The floor is
                        // set far below any real recorded age --
                        // including the youngest points in the
                        // highest-mass MIST tracks, down to ~7e-5 yr --
                        // so it never collapses distinct real ages onto
                        // a single clamped value.
                        times[i, j, f] = std::log10(std::max(track[src, ageIdx], 1e-6));
                        for (size_t k = 0; k < nQty; ++k)
                        {
        ...
        vVcrit_(vvcrit)
    {
        ...
        std::vector<double> massData = utils::readDataset1D(refGrp, "masses", "Tracks3D");
        std::ranges::sort(massData.begin(), massData.end());
        const Array1D masses(massData.data(), nmass);

        const auto fieldNames = utils::readFieldNames(refGrp, "Tracks3D");
