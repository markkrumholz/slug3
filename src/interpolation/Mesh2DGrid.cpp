...
namespace interp
{
    ...
    {
        xIntersectionDescriptor d; // Output holder
        if (!contains(x,y)) {
        ...
        {
            d.y = y;
            d.xs = s_[i,j] + std::sqrt(dx*dx + dy*dy);
        ...
        if (i == nx()-2) {
            ...
            {
                d.y = y;
                d.xs = s_[i+1,j] + std::sqrt(dx*dx + dy*dy);
                d.t = IntersectionType::spine;
                d.idx = i+1;
                d.meshExit = true;
                return std::make_pair(true, d);
    ...
    auto Mesh2DGrid::yLim(const double x) const -> std::vector<std::pair<double,double>>
    {
        ...
        else
        {
            ...
            { ++jSave_; }
            yL.push_back(y_[jSave_] + m_[iSave_,jSave_] *
                (x - x_[iSave_,jSave_]));
            ++jSave_;
        }
        ...
        while (true)
        {
            ...
            if (dx0 * dx1 < 0)
            {
	            ...
	            yL.push_back(y_[jSave_] + m_[0,jSave_] *
		            (x - x_[0,jSave_]));
            }
            ...
            {
	            // This is the regular case
	            yL.push_back(y_[jSave_] + m_[nx()-1,jSave_] *
		            (x - x_[nx()-1,jSave_]));
            } 
            else if (dx1 == 0 && jSave_ < ny()-2)
            {
        ...
        -> std::vector<xIntersectionDescriptor>
    {
        ...
        // flags
        double y = yMin;
        bool lastIntersectLeft, lastIntersectRight;
        if (startInterior) {
        ...
        else
        {
                ...
                yMin <= y_[0])
            {
                ...
                intList.push_back(
                    { y, x, IntersectionType::rib, 0, false }
                );
            ...
            else
            {
                ...
                // Add intersection point to output holder
                double s = s_[iSave_,jSave_] +
                    std::sqrt(std::pow(x - x_[iSave_,jSave_],2) +
                        std::pow(y - y_[jSave_], 2));
                intList.push_back(
                    { y, s, IntersectionType::spine, iSave_, false }
                );
            ...
            std::tuple<double, double, double>
    {
        ...
        if (searchUp)
        {
            ...
            {
                dyL = y_[jSave_] +
	                m_[iSave_,jSave_] * (x - x_[iSave_,jSave_]) - y;
            ...
            if (m_[iSave_+1,jSave_] != bigNum &&
	            !lastIntersectLeft)
            {
                dyR = y_[jSave_] +
	                m_[iSave_+1,jSave_] * (x - x_[iSave_+1,jSave_]) - y;
        ...
        ) const -> std::pair<bool, xIntersectionDescriptor>
    {
        ...
        {
            // We hit a rib
            return handleNextIntersectRib(y, x, yStop,
                endInterior, 
                lastIntersectLeft, 
                lastIntersectRight);
        } 
        ...
        {
            // We hit a spine
            const bool left = dyL < dyR;
            const double dy = std::min(dyL, dyR);
            return handleNextIntersectSpine(left,
                y, x, yStop, dy,
                endInterior,
                lastIntersectLeft,
                lastIntersectRight);
    ...
    auto Mesh2DGrid::handleNextIntersectRib(double& y,
        const double x,
        const double yStop,
        const bool endInterior,
        bool& lastIntersectLeft,
    ...
    // Handle spine intersection case
    auto Mesh2DGrid::handleNextIntersectSpine(const bool left,
        double& y,
        const double x,
        const double yStop,
        const double dy,
    ...
    const -> std::pair<bool, xIntersectionDescriptor>
    {
        ...
        // Handle left vs right spine cases
        if (left)
        {
            // We hit the left spine; this is the same whether we're
