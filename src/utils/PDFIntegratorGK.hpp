...
namespace utils
{
     ...
     *   to GKOrder::GK15.
     * @details
     * A drop-in experimental sibling to PDFIntegrator (itself
     * PDFIntegratorND<F, 1>), built on GKIntegrator's adaptive
     * Gauss-Kronrod quadrature instead of the cubature package's own
     * h-adaptive routine -- kept as a separate class specifically so
     * both can be built and timed head to head; if this one wins, it
     * is meant to replace PDFIntegrator/PDFIntegratorND outright, not
     * coexist with it indefinitely.
     *
     * Unlike PDFIntegratorND, integrate() is not safe to call
     * concurrently on a single, shared PDFIntegratorGK instance:
     * integrand() (see its own comment for why) reads the current
     * call's own real-space integration bounds back from two mutable
     * members integrate() sets just before it, rather than from a
     * per-call context object the way PDFIntegratorND's own
     * integrand()/integrandV() do. Acceptable for this class's own
     * present purpose (a timing comparison, always run single-
     * threaded); revisit if/when this class actually replaces
     * PDFIntegratorND, whose own concurrent-call support some caller
     * might depend on.
     */
    ...
    public:
        ...
        /**
         * @brief Construct a PDFIntegratorGK
         * @param p The PDF p(x) weighting the integrand; must outlive
         *   this PDFIntegratorGK, since it is stored by reference
        ...
        // Disallow copying and moving -- see PDFIntegratorND's own
        // identical comment: p_ holds a reference, so a copy would
        // alias the same PDF as the original rather than being an
        // independent instance, and there is no real need to copy or
        // move a PDFIntegratorGK in practice, since it is meant to be
        // constructed and used immediately
        PDFIntegratorGK(const PDFIntegratorGK&) = delete;
        PDFIntegratorGK(PDFIntegratorGK&&) = delete;
        auto operator=(const PDFIntegratorGK&) -> PDFIntegratorGK& = delete;
        auto operator=(PDFIntegratorGK&&) -> PDFIntegratorGK& = delete;
        ~PDFIntegratorGK() = default;
         ...
         * segment's own valid range). This clamp exists for the same
         * reason PDFIntegratorND's own integrand() clamps: exp() and
         * ln() are not exact inverses under floating point, so
        ...
        [[nodiscard]] auto integrate(double a, double b, Args&&... args) const -> std::vector<double>
        {
            ...
            if (logTransform_)
            {
                if (a <= 0.0 || b <= 0.0)
                {
                    throw std::runtime_error(
                        "PDFIntegratorGK::integrate: logTransform is set, but the "
            ...
            using IntegrandFn = decltype(&PDFIntegratorGK::template integrand<Args...>);
            const GKIntegrator<IntegrandFn, Order> integ(
                &PDFIntegratorGK::template integrand<Args...>, nInt_, maxIter_, absTol_, relTol_);
    ...
    private:
        ...
        // const, matching every other PDFIntegrator*-family class's
        // own integrate() signature. See this class's own @details
        // for the concurrency trade-off this makes.
