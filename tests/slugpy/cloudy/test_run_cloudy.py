...
def test_cluster_single_match(tmp_path):
    ...
    result = reader.run_cloudy("cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out", progress=False)
...
def test_cluster_uid_only_matches_all_times(tmp_path):
    ...
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    result = reader.run_cloudy("cluster", uid=1, save_temp=True, temp_dir=tmp_path / "out", progress=False)
...
def test_cluster_uid_list_matches_every_listed_uid(tmp_path):
    ...
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    result = reader.run_cloudy("cluster", uid=[2, 1], save_temp=True, temp_dir=tmp_path / "out", progress=False)
...
def test_cluster_uid_list_excludes_unlisted_uids(tmp_path):
    ...
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    result = reader.run_cloudy("cluster", uid=[2], save_temp=True, temp_dir=tmp_path / "out", progress=False)
    assert result is True
    assert len(_decks_in(tmp_path / "out")) == 1
...
def test_cluster_no_selector_matches_everything(tmp_path):
    ...
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    result = reader.run_cloudy("cluster", save_temp=True, temp_dir=tmp_path / "out", progress=False)
    assert result == [True, True, True]
    assert len(_decks_in(tmp_path / "out")) == 3
...
def test_cluster_no_match_raises(tmp_path):
    ...
    with pytest.raises(ValueError, match="no cluster spectra match"):
        reader.run_cloudy("cluster", uid=999, save_temp=True, temp_dir=tmp_path / "out", progress=False)
...
def test_cluster_trial_argument_raises(tmp_path):
    ...
    with pytest.raises(ValueError, match="trial is only meaningful"):
        reader.run_cloudy("cluster", trial=0, save_temp=True, temp_dir=tmp_path / "out", progress=False)


def test_cluster_feh_looked_up_by_uid(tmp_path):
    """Each cluster's own feh (not some default) is used, per its own uid."""
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    reader.run_cloudy("cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out1", progress=False)
    reader.run_cloudy("cluster", uid=2, time=1e6, save_temp=True, temp_dir=tmp_path / "out2", progress=False)
...
def test_cluster_qhi_taken_from_phot_when_available(tmp_path):
    ...
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    reader.run_cloudy("cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out", progress=False)
...
def test_cluster_missing_group_raises(tmp_path):
    ...
    with pytest.raises(ValueError, match="no cluster_spectra group"):
        reader.run_cloudy("cluster", save_temp=True, temp_dir=tmp_path / "out", progress=False)
...
def test_galaxy_fixed_feh(tmp_path):
    ...
    reader.run_cloudy("galaxy", trial=0, save_temp=True, temp_dir=tmp_path / "out", progress=False)
    path = _decks_in(tmp_path / "out")[0]
...
def test_galaxy_trial_list_matches_every_listed_trial(tmp_path):
    ...
    result = reader.run_cloudy("galaxy", trial=[1, 0], save_temp=True, temp_dir=tmp_path / "out", progress=False)
    assert result == [True, True]
    assert len(_decks_in(tmp_path / "out")) == 2
...
def test_galaxy_feh_same_for_every_matching_trial(tmp_path):
    ...
    result = reader.run_cloudy("galaxy", save_temp=True, temp_dir=tmp_path / "out", progress=False)
    assert result == [True, True]
...
def test_galaxy_uid_argument_raises(tmp_path):
    ...
    with pytest.raises(ValueError, match="uid is only meaningful"):
        reader.run_cloudy("galaxy", uid=1, save_temp=True, temp_dir=tmp_path / "out", progress=False)
...
def test_explicit_nebular_conditions(tmp_path):
    ...
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    reader.run_cloudy(
        "cluster", uid=1, time=1e6, nII=500.0 / u.cm ** 3, U=1e-3, save_temp=True, temp_dir=tmp_path / "out", progress=False)
    path = _decks_in(tmp_path / "out")[0]
    hden = _extract_float(path.read_text(), r"^hden (\S+)$")
...
def test_cloudy_path_override_takes_precedence(tmp_path, monkeypatch):
    ...
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    reader.run_cloudy(
        "cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out", cloudy_path=other_exe, progress=False)
    path = _decks_in(tmp_path / "out")[0]
    out_text = path.with_suffix(".out").read_text()
...
def test_missing_cloudy_executable_raises(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="cloudy"):
        reader.run_cloudy("cluster", uid=1, time=1e6, save_temp=True, temp_dir=out_dir, progress=False)
...
def test_default_temp_dir_is_created_next_to_the_h5_file_not_cwd(tmp_path, monkeypatch):
    """With temp_dir left at its default ("cloudy_tmp"), decks land in a cloudy_tmp subdirectory next to the h5 file, not in the current working directory."""
    other_cwd = tmp_path / "somewhere_else"
    other_cwd.mkdir()
    monkeypatch.chdir(other_cwd)

    h5_dir = tmp_path / "h5dir"
    h5_dir.mkdir()
    h5_path = _make_cluster_test_file(h5_dir, "c1.h5", qhi_in_phot=True)
    reader = slug_reader(h5_path)
    reader.run_cloudy("cluster", uid=1, time=1e6, save_temp=True, progress=False)

    assert len(_decks_in(h5_dir / "cloudy_tmp")) == 1
    assert not (other_cwd / "cloudy_tmp").exists()


def test_temp_dir_cleaned_up_by_default(tmp_path):
    """Without save_temp, temp_dir (decks and cloudy's own output) is removed once run_cloudy is done."""
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    temp_dir = tmp_path / "out"
    result = reader.run_cloudy("cluster", uid=1, time=1e6, temp_dir=temp_dir, progress=False)
    assert result is True
    assert not temp_dir.exists()


def test_save_temp_keeps_the_directory(tmp_path):
    """save_temp=True keeps temp_dir (and its contents) after run_cloudy finishes."""
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    temp_dir = tmp_path / "out"
    reader.run_cloudy("cluster", uid=1, time=1e6, temp_dir=temp_dir, save_temp=True, progress=False)
    assert len(_decks_in(temp_dir)) == 1


def test_failed_run_temp_dir_still_cleaned_up_by_default(tmp_path, _failing_cloudy_exe):
    """A failed cloudy run still leaves decks/output in temp_dir, and those are still cleaned up by default."""
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    temp_dir = tmp_path / "out"
    result = reader.run_cloudy(
        "cluster", uid=1, time=1e6, temp_dir=temp_dir, cloudy_path=_failing_cloudy_exe, progress=False)
    assert result is False
    assert not temp_dir.exists()


def test_all_skipped_never_creates_temp_dir(tmp_path):
    """A call where every match is skipped as an already-stored duplicate never creates temp_dir at all."""
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    temp_dir = tmp_path / "out"
    reader.run_cloudy("cluster", uid=1, time=1e6, temp_dir=temp_dir, save_temp=True, progress=False)
    assert temp_dir.exists()
    temp_dir2 = tmp_path / "out2"
    reader.run_cloudy("cluster", uid=1, time=1e6, temp_dir=temp_dir2, progress=False)
...
def test_max_workers_is_passed_through(tmp_path):
    ...
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    result = reader.run_cloudy("cluster", save_temp=True, temp_dir=tmp_path / "out", max_workers=1, progress=False)
    assert result == [True, True, True]
    for deck in _decks_in(tmp_path / "out"):
...
def test_single_failure_returns_false(tmp_path, _failing_cloudy_exe):
    ...
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    result = reader.run_cloudy(
        "cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out", cloudy_path=_failing_cloudy_exe, progress=False)
    assert result is False
...
def test_mixed_success_and_failure_returns_matching_bool_list(tmp_path, _mixed_cloudy_exe):
    ...
    reader = slug_reader(_make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True))
    result = reader.run_cloudy(
        "cluster", save_temp=True, temp_dir=tmp_path / "out", cloudy_path=_mixed_cloudy_exe, progress=False)
...
def test_failed_run_not_recorded(tmp_path, _mixed_cloudy_exe):
    ...
    reader = slug_reader(h5_path)
    result = reader.run_cloudy(
        "cluster", uid=2, time=1e6, save_temp=True, temp_dir=tmp_path / "out", cloudy_path=_mixed_cloudy_exe, progress=False)
    assert result is False
    with h5py.File(h5_path, "r") as f:
...
def test_mixed_batch_records_only_successes(tmp_path, _mixed_cloudy_exe):
    ...
    h5_path = _make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True)
    reader = slug_reader(h5_path)
    result = reader.run_cloudy("cluster", save_temp=True, temp_dir=tmp_path / "out", cloudy_path=_mixed_cloudy_exe, progress=False)
    assert result == [True, True, False]
    with h5py.File(h5_path, "r") as f:
        g = f["cluster_cloudy"]
...
def test_repeated_call_with_same_conditions_is_skipped_by_default(tmp_path):
    ...
    reader = slug_reader(h5_path)
    reader.run_cloudy("cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out1", progress=False)
    result = reader.run_cloudy("cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out2", progress=False)
    assert result is True
...
def test_batch_with_one_duplicate_only_reruns_the_new_one(tmp_path):
    ...
    reader = slug_reader(h5_path)
    reader.run_cloudy("cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out1", progress=False)
    result = reader.run_cloudy("cluster", uid=[1, 2], time=1e6, save_temp=True, temp_dir=tmp_path / "out2", progress=False)
    assert result == [True, True]
...
def test_repeated_call_with_different_conditions_is_not_a_duplicate(tmp_path):
    ...
    reader.run_cloudy("cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out1", progress=False)
    result = reader.run_cloudy(
        "cluster", uid=1, time=1e6, nII=1.0 / u.cm ** 3, U=1e-2, save_temp=True, temp_dir=tmp_path / "out2", progress=False)
    assert result is True
...
def test_overwrite_reruns_and_replaces_the_stored_row(tmp_path):
    ...
    reader.run_cloudy("cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out1", progress=False)
    result = reader.run_cloudy(
        "cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out2", overwrite=True, progress=False)
    assert result is True
...
def test_overwrite_without_an_existing_match_just_appends(tmp_path):
    ...
    h5_path = _make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True)
    reader = slug_reader(h5_path)
    result = reader.run_cloudy("cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out", overwrite=True, progress=False)
    assert result is True
    with h5py.File(h5_path, "r") as f:
        assert f["cluster_cloudy"]["uid"][()].tolist() == [1]
...
def test_overwrite_does_not_delete_on_a_failed_rerun(tmp_path, _mixed_cloudy_exe):
    ...
    h5_path = _make_cluster_test_file(tmp_path, "c1.h5", qhi_in_phot=True)
    reader = slug_reader(h5_path)
    reader.run_cloudy("cluster", uid=2, time=1e6, save_temp=True, temp_dir=tmp_path / "out1", progress=False)
    result = reader.run_cloudy(
        "cluster", uid=2, time=1e6, save_temp=True, temp_dir=tmp_path / "out2",
        cloudy_path=_mixed_cloudy_exe, overwrite=True, progress=False)
    assert result is False
    with h5py.File(h5_path, "r") as f:
...
def test_galaxy_run_recorded_in_galaxy_cloudy_group(tmp_path):
    ...
    reader = slug_reader(h5_path)
    result = reader.run_cloudy("galaxy", trial=0, save_temp=True, temp_dir=tmp_path / "out", progress=False)
    assert result is True
    with h5py.File(h5_path, "r") as f:
        assert "cluster_cloudy" not in f
...
def test_continuum_written_when_available(tmp_path, _continuum_cloudy_exe):
    ...
    reader = slug_reader(h5_path)
    result = reader.run_cloudy(
        "cluster", uid=1, time=1e6, save_temp=True, temp_dir=tmp_path / "out", cloudy_path=_continuum_cloudy_exe, progress=False)
    assert result is True
...
def test_lines_written_when_available(tmp_path, _continuum_cloudy_exe):
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        assert [x.decode() for x in g["line_label"][()]] == ["HI"]
