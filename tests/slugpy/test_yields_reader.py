...
def test_channels_and_models_match_deck(yields_reader: slug_reader) -> None:
...
def test_decomposed_true_by_default(yields_reader: slug_reader) -> None:
    """decomposed is True for a deck that never sets yields.channel_decomposed."""
    cy = cast(slug_yields_reader, yields_reader.cluster_yields)
    assert cy.decomposed is True


def test_decomposed_false_when_requested(not_decomposed_reader: slug_reader) -> None:
...
def test_attributes_are_read_only(yields_reader: slug_reader) -> None:
...
def test_getitem_isotope_lookup_is_case_and_space_insensitive(yields_reader: slug_reader) -> None:
...
def test_getitem_unknown_isotope_raises_keyerror(yields_reader: slug_reader) -> None:
...
def test_getitem_plain_string_decomposed_returns_full_2d_array(yields_reader: slug_reader) -> None:
...
def test_getitem_plain_string_not_decomposed_returns_1d_array(not_decomposed_reader: slug_reader) -> None:
...
def test_getitem_triple_key_not_decomposed_raises_typeerror(not_decomposed_reader: slug_reader) -> None:
...
def test_getitem_triple_exact_match_returns_1d_matching_first_column(yields_reader: slug_reader) -> None:
...
def test_getitem_triple_wildcard_channel_matches_every_column(yields_reader: slug_reader) -> None:
...
def test_getitem_triple_wildcard_model_matches_single_column(yields_reader: slug_reader) -> None:
...
def test_getitem_triple_no_match_raises_keyerror(yields_reader: slug_reader) -> None:
...
def test_getitem_triple_unknown_isotope_raises_keyerror(yields_reader: slug_reader) -> None:
...
def test_getitem_invalid_key_type_raises_typeerror(yields_reader: slug_reader) -> None:
...
def test_galaxy_yields_matches_cluster_yields_channels(yields_reader: slug_reader) -> None:
...
def test_galaxy_yields_getitem_shape(yields_reader: slug_reader) -> None:
...
def test_getitem_raw_dataset_name_falls_through(yields_reader: slug_reader) -> None:
    """A raw dataset name (not an isotope spelling) falls through to slug_group_reader.__getitem__."""
    cy = cast(slug_yields_reader, yields_reader.cluster_yields)
    assert "yields" not in [iso.upper() for iso in cy.isotopes]  # not itself an isotope spelling

    trial = cy["trial"]
    uid = cy["uid"]
    time = cast(u.Quantity, cy["time"])
    raw_yields = cast(u.Quantity, cy["yields"])
    assert isinstance(trial, np.ndarray)
    assert isinstance(uid, np.ndarray)
    assert time.unit == u.yr
    assert raw_yields.unit == u.Msun
    assert raw_yields.shape[1] == len(cy.isotopes) * len(cy.channels)
