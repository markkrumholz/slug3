/**
 * @file testPDFIntegratorGK.cpp
 * @author Mark Krumholz
 * @brief Unit tests for the PDFIntegratorGK class.
 * @date 2026-08-15
 */

#include "../../src/pdfs/PDF.hpp"
#include "../../src/pdfs/PDFFileParser.hpp"
#include "../../src/utils/PDFIntegratorGK.hpp"
...
// wrapped in a 1-element array since PDFIntegratorGK requires a
// container-valued return -- mirrors testPDFIntegrator.cpp's own
// constTwo()
...
namespace
{
    ...
    // returns m_[i] * x for every element of m_ -- mirrors
    // testPDFIntegrator.cpp's own Multiplier
    class Multiplier
    {
    public:
...
{
    const utils::PDFIntegratorGK integrator(imf, constTwo, 1U);
    ...
    {
        std::cerr << "testPDFIntegratorGK: plain-function case: expected "
...
static auto testMemberFunction(const pdfs::PDF& imf) -> int
{
    ...
    const utils::PDFIntegratorGK integrator(
    ...
    for (std::size_t i = 0; i < m.size(); ++i)
    {
        ...
        {
            std::cerr << "testPDFIntegratorGK: member-function case: at i = " << i
