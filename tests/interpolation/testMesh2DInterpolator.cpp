...
// Test specific points in x interpolation
static auto
testXInterpPoints(const std::vector<double>& ypt, 
    const std::vector<std::array<double,2>>& fpt, 
    const std::vector<std::unique_ptr<interp::Interpolator1D<nF>>>& xInterp,
    double xt)
{
...
// Test interpolation in x
static auto
testXInterp(const interp::Mesh2DInterpolator<2>& m2d,
    ...
    const std::vector<std::vector<std::array<double,2>>>& fXTest) -> int
{
        ...
        std::views::zip(xTest, xLim, yPtXTest, fXTest))
    {
        ...
        // test points
        testXInterpPoints(ypt, fpt, xInterp, xt);
    }
    return 0; // Success
}
...
auto testMesh2DInterpolator() -> int
{
    ...
    if (testXInterp(m2d, xTest, yPtXTest, xLim, fXTest) == 1)
    {
        return 1;
    }
