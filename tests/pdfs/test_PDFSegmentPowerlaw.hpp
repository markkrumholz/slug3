...
auto test_PDFSegmentPowerlaw() -> int
{
    ...
    for (double alpha : alphas) {
        ...
        if (!testUtils::approxEqual(pl(x1), norm)) {
            std::cerr << "test_PDFSegmentPowerlaw: PDF evaluation with alpha=" << alpha << " at x=1.0 failed: expected " << norm << ", got " << pl(x1) << std::endl;
            return 1;
        }
        if (!testUtils::approxEqual(pl(x2), norm * std::pow(x2, alpha))) {
            std::cerr << "test_PDFSegmentPowerlaw: PDF evaluation with alpha=" << alpha << " at x=5.0 failed: expected " << norm * std::pow(x2, alpha) << ", got " << pl(x2) << std::endl;
            return 1;
        }
        if (!testUtils::approxEqual(pl(x3), norm * std::pow(x3, alpha))) {
            std::cerr << "test_PDFSegmentPowerlaw: PDF evaluation with alpha=" << alpha << " at x=10.0 failed: expected " << norm * std::pow(x3, alpha) << ", got " << pl(x3) << std::endl;
            return 1;
        }
        if (!testUtils::approxEqual(pl(x4), 0.0)) {
            std::cerr << "test_PDFSegmentPowerlaw: PDF evaluation with alpha=" << alpha << " at x=0.5 failed: expected 0.0, got " << pl(x4) << std::endl;
            return 1;
        }
        if (!testUtils::approxEqual(pl(x5), 0.0)) {
            std::cerr << "test_PDFSegmentPowerlaw: PDF evaluation with alpha=" << alpha << " at x=15.0 failed: expected 0.0, got " << pl(x5) << std::endl;
            return 1;
        }
        ...
        if (!testUtils::approxEqual(pl.expectationValue(), expected_ev_full)) {
            std::cerr << "test_PDFSegmentPowerlaw: Expectation value calculation with alpha=" << alpha << " failed: expected " << expected_ev_full << ", got " << pl.expectationValue() << std::endl;
            return 1;
        }
        ...
        if (!testUtils::approxEqual(pl.expectationValue(a, b), expected_ev)) {
            std::cerr << "test_PDFSegmentPowerlaw: Expectation value calculation with alpha=" << alpha << " failed: expected " << expected_ev << ", got " << pl.expectationValue(a, b) << std::endl;
            return 1;
        }
        ...
        if (!testUtils::approxEqual(pl.integral(a, b), expected_integral)) {
            std::cerr << "test_PDFSegmentPowerlaw: Integral calculation with alpha=" << alpha << " failed: expected " << expected_integral << ", got " << pl.integral(a, b) << std::endl;
            return 1;
        }
        ...
        for (int i = 0; i < num_samples; ++i) {
            ...
            if (sample < a || sample > b) {
                std::cerr << "test_PDFSegmentPowerlaw: Random sampling with alpha=" << alpha << " failed: sample " << sample << " is out of bounds [" << a << ", " << b << "]" << std::endl;
                return 1;
            }
            sum_samples += sample;
        }
        ...
        if (!testUtils::approxEqual(sample_mean, expected_ev, 1e-2)) {
            std::cerr << "test_PDFSegmentPowerlaw: Random sampling with alpha=" << alpha << " mean failed: expected " << expected_ev << ", got " << sample_mean << std::endl;
            return 1;
        }
    }
