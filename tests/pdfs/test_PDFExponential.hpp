/**
 * @file test_PDFExponential.hpp
 * @author Mark Krumholz
 * @brief Unit tests for the PDFExponential class.
 * @details
 * This file contains unit tests for the PDFExponential class,
...
#ifndef TEST_PDFEXPONENTIAL_HPP
#define TEST_PDFEXPONENTIAL_HPP

#include <cmath>
#include <cstdio>
#include "../src/pdfs/PDFExponential.hpp"
#include "../tests/testUtils.hpp"

/**
 * @brief Unit test for the PDFExponential class.
 * @return 0 if the test passes, 1 if it fails.
 * @details
 * This function tests the PDFExponential class by evaluating the PDF at
...
auto test_PDFExponential() -> int
{
    ...
    pdfs::PDFExponential pe(sMin, sMax, scale, rng); // Create a PDFExponential with sMin=1, sMax=10, scale=2
    ...
    if (!testUtils::approxEqual(pe(x1), norm * std::exp(-x1 / scale))) {
        std::cerr << "test_PDFExponential: PDF evaluation at x=1.0 failed: expected " << norm * std::exp(-x1 / scale) << ", got " << pe(x1) << std::endl;
        return 1;
    }
    if (!testUtils::approxEqual(pe(x2), norm * std::exp(-x2 / scale))) {
        std::cerr << "test_PDFExponential: PDF evaluation at x=5.0 failed: expected " << norm * std::exp(-x2 / scale) << ", got " << pe(x2) << std::endl;
        return 1;
    }
    if (!testUtils::approxEqual(pe(x3), norm * std::exp(-x3 / scale))) {
        std::cerr << "test_PDFExponential: PDF evaluation at x=10.0 failed: expected " << norm * std::exp(-x3 / scale) << ", got " << pe(x3) << std::endl;
        return 1;
    }
    if (!testUtils::approxEqual(pe(x4), 0.0)) {
        std::cerr << "test_PDFExponential: PDF evaluation at x=0.5 failed: expected 0.0, got " << pe(x4) << std::endl;
        return 1;
    }
    if (!testUtils::approxEqual(pe(x5), 0.0)) {
        std::cerr << "test_PDFExponential: PDF evaluation at x=15.0 failed: expected 0.0, got " << pe(x5) << std::endl;
        return 1;
    }
    ...
    if (!testUtils::approxEqual(pe.expectationValue(), expected_expectation_value)) {
        std::cerr << "test_PDFExponential: Expectation value calculation over full range failed: expected " << expected_expectation_value << ", got " << pe.expectationValue() << std::endl;
        return 1;
    }
    ...
    if (!testUtils::approxEqual(pe.expectationValue(a, b), expected_expectation_value_range)) {
        std::cerr << "test_PDFExponential: Expectation value calculation over range [2, 8] failed: expected " << expected_expectation_value_range << ", got " << pe.expectationValue(a, b) << std::endl;
        return 1;
    }
    ...
    if (!testUtils::approxEqual(pe.integral(a, b), expected_integral)) {
        std::cerr << "test_PDFExponential: Integral calculation over range [2, 8] failed: expected " << expected_integral << ", got " << pe.integral(a, b) << std::endl;
        return 1;
    }
    ...
    for (int i = 0; i < num_samples; ++i) {
        double sample = pe.draw(a, b); // Draw a sample from the PDFExponential over the range [2, 8]
        if (sample < a || sample > b) {
            std::cerr << "test_PDFExponential: Sample drawn from PDFExponential is out of range: got " << sample << ", expected between " << a << " and " << b << std::endl;
            return 1;
        }
    ...
    if (!testUtils::approxEqual(sample_mean, expected_expectation_value_range, 0.01)) {
        std::cerr << "test_PDFExponential: Sample mean from drawn samples does not match expected expectation value: expected " << expected_expectation_value_range << ", got " << sample_mean << std::endl;
        return 1;
    }
