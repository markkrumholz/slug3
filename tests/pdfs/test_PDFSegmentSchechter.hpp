...
auto test_PDFSegmentSchechter() -> int
{
    ...
    for (double alpha : alphas) {
        ...
        if (!testUtils::approxEqual(ps(x1), norm  * std::exp(-x1 / sStar))) {
            std::cerr << "test_PDFSegmentSchechter: PDF evaluation with alpha=" << alpha << " at x=1.0 failed: expected " << norm << ", got " << ps(x1) << std::endl;
            return 1;
        }
        if (!testUtils::approxEqual(ps(x2), norm * std::pow(x2, alpha) * std::exp(-x2 / sStar))) {
            std::cerr << "test_PDFSegmentSchechter: PDF evaluation with alpha=" << alpha << " at x=5.0 failed: expected " << norm * std::pow(x2, alpha) << ", got " << ps(x2) << std::endl;
            return 1;
        }
        if (!testUtils::approxEqual(ps(x3), norm * std::pow(x3, alpha) * std::exp(-x3 / sStar))) {
            std::cerr << "test_PDFSegmentSchechter: PDF evaluation with alpha=" << alpha << " at x=10.0 failed: expected " << norm * std::pow(x3, alpha) << ", got " << ps(x3) << std::endl;
            return 1;
        }
        if (!testUtils::approxEqual(ps(x4), 0.0)) {
            std::cerr << "test_PDFSegmentSchechter: PDF evaluation with alpha=" << alpha << " at x=0.5 failed: expected 0.0, got " << ps(x4) << std::endl;
            return 1;
        }
        if (!testUtils::approxEqual(ps(x5), 0.0)) {
            std::cerr << "test_PDFSegmentSchechter: PDF evaluation with alpha=" << alpha << " at x=15.0 failed: expected 0.0, got " << ps(x5) << std::endl;
            return 1;
        }
        ...
        if (!testUtils::approxEqual(ps.expectationValue(), expected_ev_full)) {
            std::cerr << "test_PDFSegmentSchechter: Expectation value calculation with alpha=" << alpha << " failed: expected " << expected_ev_full << ", got " << ps.expectationValue() << std::endl;
            return 1;
        }
        ...
        if (!testUtils::approxEqual(ps.expectationValue(a, b), expected_ev)) {
            std::cerr << "test_PDFSegmentSchechter: Expectation value calculation with alpha=" << alpha << " failed: expected " << expected_ev << ", got " << ps.expectationValue(a, b) << std::endl;
            return 1;
        }
        ...
        if (!testUtils::approxEqual(ps.integral(a, b), expected_integral)) {
            std::cerr << "test_PDFSegmentSchechter: Integral calculation with alpha=" << alpha << " failed: expected " << expected_integral << ", got " << ps.integral(a, b) << std::endl;
            return 1;
        }
        ...
        for (int i = 0; i < num_samples; ++i) {
            ...
            if (sample < a || sample > b) {
                std::cerr << "test_PDFSegmentSchechter: Random sampling with alpha=" << alpha << " failed: sample " << sample << " is out of bounds [" << a << ", " << b << "]" << std::endl;
                return 1;
            }
            sum_samples += sample;
        }
        ...
        if (!testUtils::approxEqual(sample_mean, expected_ev, 1e-2)) {
            std::cerr << "test_PDFSegmentSchechter: Random sampling with alpha=" << alpha << " mean failed: expected " << expected_ev << ", got " << sample_mean << std::endl;
            return 1;
        }
    }
