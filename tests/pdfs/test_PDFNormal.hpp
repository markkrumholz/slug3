/**
 * @file test_PDFNormal.hpp
 * @author Mark Krumholz
 * @brief Unit tests for the PDFNormal class.
 * @details
 * This file contains unit tests for the PDFNormal class defined 
 * in the src/pdfs directory.
 * @date 2024-06-12
 */

#ifndef TEST_PDFNORMAL_HPP
#define TEST_PDFNORMAL_HPP

#include <cmath>
#include <cstdio>
#include "../src/pdfs/PDFNormal.hpp"
#include "../tests/testUtils.hpp"

/**
 * @brief Unit test for the PDFNormal class.
 * @return 0 if the test passes, 1 if it fails.
 * @details
 * This function tests the PDFNormal class by evaluating the PDF at
...
auto test_PDFNormal() -> int
{
    ...
    pdfs::PDFNormal pn(sMin, sMax, mean, stdDev, rng); // Create a PDFNormal with mean=5, stdDev=1, sMin=1, sMax=6
    ...
    double norm = std::sqrt(2.0 / M_PI) / stdDev /
        (std::erf((sMax - mean) / (stdDev * std::sqrt(2))) - 
         std::erf((sMin - mean) / (stdDev * std::sqrt(2)))); // Normalization constant for the PDFNormal
    if (!testUtils::approxEqual(pn(x1), norm * std::exp(-0.5 * std::pow((x1 - mean) / stdDev, 2)))) {
        std::cerr << "test_PDFNormal: PDF evaluation at x=1.0 failed: expected " << norm * 
    ...
    if (!testUtils::approxEqual(pn(x2), norm * std::exp(-0.5 * std::pow((x2 - mean) / stdDev, 2)))) {
        std::cerr << "test_PDFNormal: PDF evaluation at x=5.0 failed: expected " << norm * 
    ...
    if (!testUtils::approxEqual(pn(x3), norm * std::exp(-0.5 * std::pow((x3 - mean) / stdDev, 2)))) {
        std::cerr << "test_PDFNormal: PDF evaluation at x=10.0 failed: expected " << norm * 
            std::exp(-0.5 * std::pow((x3 - mean) / stdDev, 2)) << ", got " << pn(x3) << std::endl;
        return 1;
    }
    if (!testUtils::approxEqual(pn(x4), 0.0)) {
        std::cerr << "test_PDFNormal: PDF evaluation at x=0.5 failed: expected 0.0, got " << pn(x4) << std::endl;
        return 1;
    }
    if (!testUtils::approxEqual(pn(x5), 0.0)) {
        std::cerr << "test_PDFNormal: PDF evaluation at x=15.0 failed: expected 0.0, got " << pn(x5) << std::endl;
        return 1;
    }
    ...
    if (!testUtils::approxEqual(pn.expectationValue(), expected_expectation_value)) {
        std::cerr << "test_PDFNormal: Expectation value calculation over full range failed: expected " << expected_expectation_value << ", got " << pn.expectationValue() << std::endl;
        return 1;
    }
    ...
    if (!testUtils::approxEqual(pn.expectationValue(a, b), expected_expectation_value_range)) {
        std::cerr << "test_PDFNormal: Expectation value calculation over range [2.0, 5.0] failed: expected " << expected_expectation_value_range << ", got " << pn.expectationValue(a, b) << std::endl;
        return 1;
    }
    ...
    if (!testUtils::approxEqual(pn.integral(a, b), expected_integral)) {
        std::cerr << "test_PDFNormal: Integral calculation over range [2.0, 5.0] failed: expected " << expected_integral << ", got " << pn.integral(a, b) << std::endl;
        return 1;
    }
    ...
    for (int i = 0; i < num_samples; ++i) {
        double sample = pn.draw(a, b); // Draw a sample from the PDFNormal over the range [2, 5]
        if (sample < a || sample > b) {
            std::cerr << "test_PDFNormal: Sample drawn from PDFNormal is out of range: got " << sample << ", expected between " << a << " and " << b << std::endl;
            return 1;
        }
    ...
    if (!testUtils::approxEqual(sample_mean, expected_expectation_value_range, 0.01)) {
        std::cerr << "test_PDFNormal: Sample mean from drawn samples does not match expected expectation value: expected " << expected_expectation_value_range << ", got " << sample_mean << std::endl;
        return 1;
    }
