...
namespace
{
    ...
    // fetched them
    const std::array<std::string, 15> requiredDataFiles = { { // NOLINT(bugprone-throwing-static-initialization,cert-err58-cpp) -- built from fixed string literals, so allocation failure aside (which would abort regardless), this can't actually throw
