...
* ``sb99_modc``, ``sb99_mode``, ``sb99_modp``, ``sb99_mods``: these are legacy tracks from the `starburst99 <https://massivestars.stsci.edu/starburst99/docs/default.htm>`_ code; ``modc`` and ``mode`` are "old" Geneva models with standard and 2x standard mass loss, respectively, and ``modp`` and ``mods`` are "old" Padova models with and without TP-AGB stars. All these models span [Fe/H] = -1.69897 to 0.39794, with no rotation and [alpha/Fe] = 0. These models are provided for completeness and testing against results derived with starburst99 or earlier SLUG versions, but are not recommended for production work due to their sparse mass sampling.
