...
* ``Geneva``: the `Geneva/SYCLIST <https://github.com/GESEG/SYCLIST>`_ tracks. These tracks are available for stars with masses of :math:`0.8-120\,\mathrm{M}_\odot` at :math:`v/v_\mathrm{crit} = 0` and 0.4, and [Fe/H] = -0.85 to 0.0 (Z = 0.002, 0.006, and 0.014). Since these tracks are not parameterized by a single [alpha/Fe] value, the ``alphaFe`` parameter is ignored for this track set. Two companion track sets, ``Geneva_Z0004`` and ``Geneva_Z014`` below, extend the same underlying grid with extra mass coverage at its lowest and highest metallicity, respectively, at the cost of not sharing ``Geneva``'s own common mass grid (see those entries below for why).
* ``Geneva_Z0004``: the same Geneva/SYCLIST tracks as ``Geneva`` above, but at the single lowest metallicity available, Z = 0.0004 ([Fe/H] = -1.54), split into its own track set because its mass grid, :math:`1.7-120\,\mathrm{M}_\odot`, does not reach as low a minimum mass as ``Geneva``'s own :math:`0.8\,\mathrm{M}_\odot`.
* ``Geneva_Z014``: the same Geneva/SYCLIST tracks as ``Geneva`` above, but at the single highest (solar) metallicity, Z = 0.014 ([Fe/H] = 0.0), split into its own track set because it retains seven extra fine-grid masses between 8 and roughly :math:`12\,\mathrm{M}_\odot` that were dropped from ``Geneva`` itself; spans the same :math:`0.8-120\,\mathrm{M}_\odot` range as ``Geneva``, but with 31 mass points rather than 24.
* ``sb99_modc``, ``sb99_mode``, ``sb99_modp``, ``sb99_mods``: these are legacy tracks from the `starburst99 <https://massivestars.stsci.edu/starburst99/docs/default.htm>`_ code; ``modc`` and ``mode`` are "old" Geneva models with standard and 2x standard mass loss, respectively, and ``modp`` and ``mods`` are "old" Padova models with and without TP-AGB stars. The Geneva models span [Fe/H] = -1.30103 to 0.30103 and the Padova models span [Fe/H] = -1.69897 to 0.39794, all with no rotation and [alpha/Fe] = 0. These models are provided for completeness and testing against results derived with starburst99 or earlier SLUG versions, but are not recommended for production work due to their sparse mass sampling.
