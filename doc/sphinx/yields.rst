...
currently included in SLUG are:
    ...
    * ``agb``: winds from asymptotic giant branch (AGB) stars
...
* For the ``agb`` channel:
  ...
  * ``agb-composite``: a composite model that combines all four of the component AGB yield sources listed below into a single recommended grid. The default mass range is :math:`0.9 - 9\,\mathrm{M}_\odot` (35 tabulated masses), and the yields are available at 19 [Fe/H] values ranging from :math:`-8.15` to :math:`+0.85` (corresponding to metallicities :math:`Z \approx 10^{-10}` to :math:`Z \approx 0.10`). Within this grid, each (mass, [Fe/H]) cell is filled from whichever of the four component sources has the highest priority for that cell, with the priority order being Karakas & Lugaro :math:`>` Doherty et al. :math:`>` Gil-Pons et al. :math:`>` Cinquegrana & Karakas. In practice the sources cover largely non-overlapping regions of the (mass, [Fe/H]) plane: Gil-Pons et al. provides the very-low-metallicity cells ([Fe/H] :math:`< -4`), Karakas & Lugaro provides the bulk of the AGB grid (:math:`-1.85 \leq` [Fe/H] :math:`\leq +0.33`), Doherty et al. extends the highest-mass cells (:math:`m \gtrsim 6.5\,\mathrm{M}_\odot`) into the super-AGB regime, and Cinquegrana & Karakas provides the super-solar metallicity cells ([Fe/H] :math:`> +0.45`). For cells where the preferred source has no model, yields are linearly interpolated in mass from the two bracketing models in the source's own grid; cells outside any source's mass range at a given [Fe/H] receive zero yield. This model is the recommended choice for most applications. References: `Karakas & Lugaro (2016) <https://ui.adsabs.harvard.edu/abs/2016ApJ...825...26K/abstract>`_, `Karakas et al. (2018) <https://ui.adsabs.harvard.edu/abs/2018MNRAS.477..421K/abstract>`_, `Doherty et al. (2014) <https://ui.adsabs.harvard.edu/abs/2014MNRAS.437..195D/abstract>`_, `Gil-Pons et al. (2022) <https://ui.adsabs.harvard.edu/abs/2022A%26A...668A.100G/abstract>`_, `Cinquegrana & Karakas (2022) <https://ui.adsabs.harvard.edu/abs/2022MNRAS.510.1557C/abstract>`_.
  * ``karakas-pmz0``, ``karakas-pmz1e-4``, ``karakas-pmz2e-4``, ``karakas-pmz5e-4``, ``karakas-pmz1e-3``, ``karakas-pmz2e-3``, ``karakas-pmz4e-3``, ``karakas-pmz6e-3``: the AGB yields of `Karakas & Lugaro (2016) <https://ui.adsabs.harvard.edu/abs/2016ApJ...825...26K/abstract>`_ and `Karakas et al. (2018) <https://ui.adsabs.harvard.edu/abs/2018MNRAS.477..421K/abstract>`_, at eight values of the partial mixing zone (PMZ) parameter that controls the extent of proton ingestion into the intershell and therefore the efficiency of the s-process. The PMZ parameter values are :math:`0`, :math:`10^{-4}`, :math:`2\times10^{-4}`, :math:`5\times10^{-4}`, :math:`10^{-3}`, :math:`2\times10^{-3}`, :math:`4\times10^{-3}`, and :math:`6\times10^{-3}\,\mathrm{M}_\odot`. Not all PMZ values are available at all masses and metallicities; the grids for the eight models therefore differ. The models cover [Fe/H] in the range :math:`-1.85` to :math:`+0.33` (7 metallicities, :math:`Z = 0.0002` to :math:`0.03`), with the full mass range approximately :math:`0.9 - 8\,\mathrm{M}_\odot` but varying somewhat by metallicity and PMZ. The ``agb-composite`` model described above applies a set of empirically motivated PMZ selection rules to automatically choose the appropriate PMZ value at each (mass, [Fe/H]) cell; users who want direct control over the PMZ can instead use one of these individual files.
  * ``doherty14``: the super-AGB yields of `Doherty et al. (2014) <https://ui.adsabs.harvard.edu/abs/2014MNRAS.437..195D/abstract>`_ for stars in the mass range :math:`6.5 - 9\,\mathrm{M}_\odot` (6 tabulated masses). These models use the Vassiliadis & Wood (1993) mass-loss prescription and cover [Fe/H] = :math:`-0.70`, :math:`-0.40`, and :math:`0.0` (3 metallicities, :math:`Z = 0.004`, :math:`0.008`, and :math:`0.02` with :math:`Z_\odot = 0.02` as adopted by Doherty et al.). Stars in this mass range ignite carbon off-center and become super-AGB stars; their yields are dominated by dredge-up products and mass loss from the envelope prior to collapse. The ``agb-composite`` model uses these yields at masses above the upper limit of the Karakas & Lugaro grid. Note that the three metallicities do not share exactly the same mass grid (the lowest metallicity is missing the two highest masses, for example); the missing entries are padded with zero yield to satisfy the single-shared-mass-grid file format (see :ref:`ssec-adding-yield-models`), so a user selecting ``doherty14`` directly, rather than through ``agb-composite``, should be aware that SLUG's mass interpolation will taper linearly toward these padded zeros rather than truly having no data there. This does not affect ``agb-composite``, which always prefers real Karakas & Lugaro coverage over Doherty et al. at masses where both are tabulated.
  * ``gilpons22``: the AGB yields of `Gil-Pons et al. (2022) <https://ui.adsabs.harvard.edu/abs/2022A%26A...668A.100G/abstract>`_ for stars at very low metallicities. The default mass range is :math:`3 - 8.5\,\mathrm{M}_\odot` (up to 8 tabulated masses), and the yields are available at 4 [Fe/H] values ranging from :math:`-8.15` to :math:`-4.15` (corresponding to metallicities :math:`Z = 10^{-10}` to :math:`10^{-6}` on a scale where :math:`Z_\odot = 0.014`). These extremely low metallicities are relevant for modelling early-Universe or Population~III-enriched environments.
  * ``cinquegrana22``: the super-solar AGB yields of `Cinquegrana & Karakas (2022) <https://ui.adsabs.harvard.edu/abs/2022MNRAS.510.1557C/abstract>`_. The default mass range is :math:`1 - 8\,\mathrm{M}_\odot` (15 tabulated masses), and the yields are available at 7 [Fe/H] values ranging from :math:`+0.46` to :math:`+0.85` (corresponding to :math:`Z = 0.04` to :math:`0.10` on a scale where :math:`Z_\odot = 0.014`). These models extend AGB yield coverage to super-solar metallicities, which are relevant for modelling metal-rich environments such as the Galactic bulge. No PMZ variations are provided for this grid.
...
i.e. one of ``ccsn``, ``massive_star_winds``, or ``agb``. A file may contain groups
for one or more channels; for example ``kobayashi06_11.h5`` contains only ``ccsn``,
whereas ``sukhbold16.h5`` contains both ``ccsn`` and ``massive_star_winds``. Each channel group must have:
